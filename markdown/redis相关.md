...
+ RDB
  + 命令：SAVE\BGSAVE
  + save期间阻塞服务器，不能执行其它命令
  + fork子进程，由子进程进行快照
  + 优缺点
    + 全量备份、适合容灾恢复
    + BGSAVE会开辟子进程，主进程不需要进行IO操作
    + 恢复大数据集的时候速度比AOF快
    + 持久化期间修改的数据不会被保存，可能丢失数据
+ AOF
  + 命令：bgwriteaof
  + 类似日志记录、把命令追加到文件中
  + 每隔一秒写一次
...
1、过期时间在redis里面存在的形式：由于redis是key-value形式存储数据的，所以我们大概可以理解redis数据库的结构就是一个大map，存储一堆key-value。过期时间其实也是以key-value的形式存在于数据库里面，类似一张单独的哈希表，key与存储值的key一样，value是过期的具体时间（timestamp）。所以两者是分开存储的

2、判断键是否过期：使用get命令查找某个key的value，检查key是否存在于过期字典，如果存在，则获取key的过期时间，如果当前时间大于key存储的过期时间，则说明键已经过期，否则说明没有过期
